...
@Component({
  selector: 'app-color-war',
  imports: [],
  templateUrl: './color-war.html',
  styleUrl: './color-war.scss',
})
export class ColorWar implements OnInit, OnDestroy {
  ...
  private initPalette() {
    for (let i = 0; i < N_COLORS; i++) {
      ...
      const [r, g, b] = hsvToRgb(hue, 1.0, 1.0); // S=1.0 (max nasycenie), V=1.0 (max jasność)
      const offset = i * 4;
  ...
  noiseShader: string = `
    ...
    fn fs_main(@builtin(position) pos: vec4f) -> FragmentOutput {
      ...
      var winnerColor = textureLoad(feedbackTexture, coords, 0); // Domyślnie zostaje stary kolor
      var maxPower = -1.0;
